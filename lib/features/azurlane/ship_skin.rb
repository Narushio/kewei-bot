module Features::Azurlane::ShipSkin
  ...
  def ship_skin_card(name: nil, ship_id: nil, index: 1)
    ...
    begin
      ...
      width = bg.properties["png:IHDR.width,height"].split(",")[0].strip
      height = bg.properties["png:IHDR.width,height"].split(",")[1].strip
      ...
      if File.exist?(chibi_path)
        ...
        skin_card = bg.composite(image.resize_to_fit(width, height), CenterGravity, SrcOverCompositeOp)
